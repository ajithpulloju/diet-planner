...
router.get('/log/:date', async (req, res) => {
  try {
    ...
    if (!log) log = { breakfast: [], lunch: [], snacks: [], dinner: [] };
...
router.post('/log/:date/:slot', async (req, res) => {
  try {
    ...
    if (!log) log = new NutritionLog({ userId: req.user._id, date, breakfast: [], lunch: [], snacks: [], dinner: [] });
...
router.get('/week', async (req, res) => {
  try {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      logs[ds] = log || { breakfast: [], lunch: [], snacks: [], dinner: [] };
    }
